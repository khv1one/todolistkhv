...
import play.api.mvc.{AnyContent, BodyParsers, Cookie, Request, RequestHeader, Results, Session}
...
class AuthenticationControllerSpec
  ...
  //implicit val requestHeader: RequestHeader = app.injector.instanceOf[RequestHeader]
  ...
  val userAction = new UserActionMock(Option(user))
  ...
  "login" should {
    ...
    when( userRepo.userByNameAndPassword(user.username, user.password) )
      .thenReturn(OptionT(Future(Option(user))))
    ...
    "check NF" in {
      when( userRepo.userByNameAndPassword(user.username, user.password) )
        .thenReturn(OptionT(Future(Option.empty)))
      val method = controller.login(user.username, user.password).apply(request)
  ...
  "logout" should {
    ...
    "check new session" in { // не работает
      ...
      //s.get(GlobalKeys.SESSION_USER_NAME_KEY) mustBe Option.empty
    }
  }
}
